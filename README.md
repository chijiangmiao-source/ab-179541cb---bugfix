...
3. 以匹配 DP 为门控，逐掩码合成“奇度锚点的一条最短路”与“剩余子问题**全部**最优 T-join”的**对称差**（每个子掩码保留全部不同的最优边集，仅留一个代表会漏算嵌套并列方案），用整数掩码天然去重，得到**全部不同的最优 T-join**（18 奇度节点最坏情形亚秒级）；
...
- `tests/test_solver.py`：确定性用例（含六节点双并列方案管网与录入顺序无关性）+ 60 组随机图对**逐子集暴力枚举 oracle** 的交叉验证（同优数量、分类、规范集、路线）；
- `tests/test_api.py`：HTTP 序列化、错误定位、畸形请求，以及对真实 gunicorn 执行 `GET /health` 与 `POST /api/audit` 的冒烟测试。
