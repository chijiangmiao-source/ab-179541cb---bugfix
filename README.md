...
1. 管段按标识字典序排列，均分成两半（各 ≤16 条），用 Gray 码枚举每一半的全部子集，记录各子集的奇度位向量与总权值；
2. 两半按 (奇度位向量, 权值) 聚合，以位向量互补配对，精确求出最小增程与**不同最优边集的数量**——同一集合即使有多种“最短路/匹配”分解也只计一次；
3. 逐条边统计含该边的最优集合数，据此判定必重复 / 可重复 / 从不重复；
4. 按标识顺序逐条做“钉住前缀后是否仍存在最优解”的可行性裁决（`0` 可行即钉 `0`），得到 `0` 优先规范位向量与规范集合；
